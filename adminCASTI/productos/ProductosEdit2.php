...
//Carga valores del registro cuando esta EDITANDO	
	if (isset($_POST['id'])) {
		...
		$idProducto = uniqid($_SESSION['InicioSesion']);		
	}
...
$("#BotonUpload").click(function(){
  ...
  <div id="Mensaje"></div>
    ...
    <div class="FormDataField">
      ...
      <input class="DataField F6" type="text" name="Referencia" id="Referencia" value="<?php echo (isset($_POST['id']) ? utf8_encode($row['Referencia']) : ''); ?>">
    </div>
    <div class="FormDataField">
      <label class="DataName">Producto:</label>
      <input class="DataField F6 Required" type="text" name="Producto" id="Producto" value="<?php echo (isset($_POST['id']) ? utf8_encode($row['Producto']) : ''); ?>">
    </div>
    ...
    <div class="FormTextareaField ACenter">
    	<?php
				if (is_file("../../images/productos/".$idProducto.".".$row['TipoArchivo'])) { 
        	$src = "/".$_SESSION['Web']."/images/productos/".$idProducto.".".$row['TipoArchivo']."?".time();
				} else {
