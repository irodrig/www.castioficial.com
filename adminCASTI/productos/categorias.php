<?php
	...
	$whereCond = "WHERE idCategoria!=''";
	if (isset($_GET['bCategoria']) AND $_GET['bCategoria'] != "") {
		$whereCond .= " AND Categoria LIKE '%".$_GET['bCategoria']."%'";
	} 
	$orderQry = "OrdenWeb, Categoria";
	...
	$qry = "SELECT * FROM AL_Categorias ".$whereCond;
	$rst = $MainMySQL->query($qry);
	$numRows = $rst->num_rows;
	$qry = "SELECT * FROM AL_Categorias ".$whereCond." ORDER BY ".$orderQry.$rangoSQL;
	$rst = $MainMySQL->query($qry);
    ...
    <div id="Acciones">
      <ul class="Botones">
        ...
        <form id="fOrden" name="fOrden" action="<?php echo $_SERVER['PHP_SELF']."?".$queryString; ?>" method="post">
          <select name="qOrden">
            <option value="OrdenWeb, Categoria"<?php if ("OrdenWeb, Categoria"==$_POST['qOrden']) {echo "SELECTED";} ?>>ORDEN WEB</option>
            <option value="Categoria"<?php if ("Categoria"==$_POST['qOrden']) {echo "SELECTED";} ?>>CATEGORIA</option>
          </select>
      ...
      <div id="DatosBusqueda">
        <div class="BusquedaCol_1_3 SeparadorBusquedaCol">
          <div class="BusquedaParametro">
            ...
            <input name="bCategoria" type="text" id="bCategoria" value="<?php echo (isset($_GET['bCategoria']) ? $_GET['bCategoria'] :  NULL); ?>"/>
          </div>
				</div>
      </div>
    </form>
        ...
        <tbody class="scrollContent">
          <?php 
            ...
            while ($row = $rst->fetch_array()) {
              $i++; 
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['idCategoria'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:50px"><?php echo Numero($row['OrdenWeb'],0); ?></td>
                <td style="width:500px"><?php echo utf8_encode($row['Categoria']); ?></td>
                <td style="width:500px"><?php echo utf8_encode($row['Keywords']); ?></td>
                <td style="width:50px" ><?php echo ($row['Activo'] ? 'SI' : ''); ?></td>
                <td style="width:100px" align="center">
									<?php
                    if (is_file("../../images/categorias/".$row['idCategoria'].".".$row['TipoArchivo'])) { 
                      $src = "/".$_SESSION['Web']."/images/categorias/".$row['idCategoria'].".".$row['TipoArchivo']."?".time();
                    } else {
