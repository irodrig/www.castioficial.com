...
//Carga valores del registro cuando esta EDITANDO	
	if (isset($_POST['id'])) {
		$qry = "SELECT * FROM AL_Familias WHERE idFamilia = '".$_POST['id']."'";
		$rst = $MySQL->query($qry);
		$row = $rst->fetch_array();
		$idFamilia = $row['idFamilia'];
	} else {
		$idFamilia = uniqid($_SESSION['InicioSesion']);		
	}
...
$(document).ready(function(e) {
	...
	attrCategorias = {
		idElemento: "#cmbCategoria", 
		Etiqueta: "Categoria",
		Tabla: "AL_Categorias",
		Origen: "idCategoria, Categoria",
		Destino: "idCategoria, Categoria",
		Clase: "F6",
		SoloActivos: 0, 
	}
...
$("#BotonUpload").click(function(){
  ...
  <div id="Mensaje"></div>
    ...
    <div class="FormDataField">
      <label class="DataName">Familias:</label>
      <input class="DataField F6 Required" type="text" name="Familia" id="Familia" value="<?php echo (isset($_POST['id']) ? utf8_encode($row['Familia']) : ''); ?>">
    </div>
    <div class="FormDataField" id="cmbCategoria" valor="<?php echo (isset($_POST['id']) ? $row['idCategoria'] : ''); ?>"></div>
    <div class="FormDataField">
    ...
    <div class="FormTextareaField ACenter">
    	<?php
				if (is_file("../../images/familias/".$idFamilia.".".$row['TipoArchivo'])) { 
        	$src = "/".$_SESSION['Web']."/images/familias/".$idFamilia.".".$row['TipoArchivo']."?".time();
				} else {
