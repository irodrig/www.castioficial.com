<?php
	...
	$whereCond = "WHERE idColaborador!=''";
	if (isset($_GET['bNombre']) AND $_GET['bNombre'] != "") {
		$whereCond .= " AND Nombre LIKE '%".$_GET['bNombre']."%'";
	} 
	$orderQry = "Fecha DESC";
	...
	$qry = "SELECT * FROM WE_Colaboradores ".$whereCond;
	$rst = $MainMySQL->query($qry);
	$numRows = $rst->num_rows;
	$qry = "SELECT * FROM WE_Colaboradores ".$whereCond." ORDER BY ".$orderQry.$rangoSQL;
	$rst = $MainMySQL->query($qry);
      ...
      <div id="DatosBusqueda">
        <div class="BusquedaCol_1_3 SeparadorBusquedaCol">
          <div class="BusquedaParametro">
            <label>NOMBRE</label>
            <input name="bNombre" type="text" id="bNombre" value="<?php echo (isset($_GET['bNombre']) ? $_GET['bNombre'] :  NULL); ?>"/>
          </div>
				</div>
      </div>
    </form>
        ...
        <tbody class="scrollContent">
          <?php 
            ...
            while ($row = $rst->fetch_array()) {
              $i++; 
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['idColaborador'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:400px"><?php echo utf8_encode($row['Nombre']); ?></td>
                <td style="width:350px"><?php echo utf8_encode($row['Apellido']); ?></td>
                <td style="width:400px"><?php echo utf8_encode($row['Email']); ?></td>
              </tr>
          <?php 
            }
            $rst->free();
