<?php
	...
	$whereCond = "WHERE idfaq!=''";
	if (isset($_GET['bPregunta']) AND $_GET['bPregunta'] != "") {
		$whereCond .= " AND Pregunta LIKE '%".$_GET['bPregunta']."%'";
	} 
	...
	$qry = "SELECT * FROM WE_Faqs ".$whereCond;
	$rst = $MainMySQL->query($qry);
	$numRows = $rst->num_rows;
	$qry = "SELECT * FROM WE_Faqs ".$whereCond." ORDER BY ".$orderQry.$rangoSQL;
	$rst = $MainMySQL->query($qry);
      ...
      <ul class="Botones">
        ...
        <form id="fOrden" name="fOrden" action="<?php echo $_SERVER['PHP_SELF']."?".$queryString; ?>" method="post">
          <select name="qOrden">
            ...
            <option value="Pregunta"<?php if ("Pregunta"==$_POST['qOrden']) {echo "SELECTED";} ?>>PREGUNTA</option>
          </select>
      ...
      <div id="DatosBusqueda">
        <div class="BusquedaCol_1_3 SeparadorBusquedaCol">
          <div class="BusquedaParametro">
            ...
            <input name="bPregunta" type="text" id="bPregunta" value="<?php echo (isset($_GET['bPregunta']) ? $_GET['bPregunta'] :  NULL); ?>"/>
          </div>
				</div>
        <div class="BusquedaCol_1_3 SeparadorBusquedaCol">
          <div class="BusquedaParametro">
        ...
        <tbody class="scrollContent">
          <?php 
            ...
            while ($row = $rst->fetch_array()) {
              $i++; 
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['idFaq'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:50px"><?php echo cambiaf_a_normal($row['Fecha']) ; ?></td>
                <td style="width:550px"><?php echo utf8_encode($row['Pregunta']); ?></td>
                <td style="width:450px"><?php echo utf8_encode($row['Keywords']); ?></td>
                ...
                <td style="width:100px">
                   ...
                   if (is_file($srv."/images/faqs/".$row['idFaq'].".".$row['TipoArchivo'])) { 
                      $src = "/".$_SESSION['Web']."/images/faqs/".$row['idFaq'].".".$row['TipoArchivo']."?".time();
                   } else {
