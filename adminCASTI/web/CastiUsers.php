<?php
	...
	$qry = "SELECT * FROM WE_Usuarios ".$whereCond;
	$rst = $MainMySQL->query($qry);
	$numRows = $rst->num_rows;
	$qry = "SELECT * FROM WE_Usuarios ".$whereCond." ORDER BY ".$orderQry.$rangoSQL;
	$rst = $MainMySQL->query($qry);
        ...
        <tbody class="scrollContent">
          ...
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['idUsuario'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:100px"><?php echo (substr($row['Fecha'],0,10)) ; ?></td>
                <td style="width:350px"><?php echo utf8_encode($row['Nombre']); ?></td>
