<?php
	...
	$qry = "SELECT * FROM WE_Noticias ".$whereCond;
	$rst = $MainMySQL->query($qry);
	$numRows = $rst->num_rows;
	$qry = "SELECT * FROM WE_Noticias ".$whereCond." ORDER BY ".$orderQry.$rangoSQL;
	$rst = $MainMySQL->query($qry);
            ...
            while ($row = $rst->fetch_array()) {
              $i++; 
							...
							if ($_SERVER['SERVER_NAME'] == 'localhost') {
								$directorio = "../../../PRS/CASTI/images/noticias/".$row['idNoticia'];
//								$archivos = scandir();
							} else {
								$directorio = "../../CASTI/images/noticias/".$row['idNoticia'];
//								$archivos = scandir("../../CASTI/images/noticias/".$row['idNoticia']);
							}
          ...
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['idNoticia'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:50px"><?php echo $fec; ?></td>
                <td style="width:400px"><?php echo utf8_encode($row['Titular']); ?></td>
                <td style="width:400px"><?php echo utf8_encode($row['Avance']); ?></td>
...
<!--                <td style="width:100px">
                   ...
                   if (is_file($srv."/images/noticias/".$row['idNoticia'].".".$row['TipoArchivo'])) { 
                      $src = "/".$_SESSION['Web']."/images/noticias/".$row['idNoticia'].".".$row['TipoArchivo']."?".time();
                   } else {
