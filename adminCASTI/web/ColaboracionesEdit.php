...
//Carga valores del registro cuando esta EDITANDO	
	if (isset($_POST['id'])) {
		$qry = "SELECT * FROM WE_Colaboraciones WHERE idColaboracion = '".$_POST['id']."'";
		$rst = $MySQL->query($qry);
		$row = $rst->fetch_array();
		$idColaboracion = $row['idColaboracion'];
	} else {
		$row['Activo'] = 1;
		$idColaboracion = uniqid($_SESSION['InicioSesion']);		
	}
	if ($_SERVER['SERVER_NAME'] != 'localhost') {
//		$carpeta = "../../CASTI/images/Colaboraciones";
		$carpeta = "https://castioficial.com/CASTI/images/Colaboraciones";
	} else {
		$carpeta = "../../../PRS/CASTI/images/Colaboraciones";
	}
...
$(document).ready(function(e) {
	...
	idColaboracion = "<?php echo $idColaboracion ?>";
	inicializarEdicion(idEdicion, DocumentoActual);

	attrColaboradores = {
		idElemento: "#cmbColaborador",
		Etiqueta: "Colaborador",
		Tabla: "WE_Colaboradores",
		Origen: "idColaborador, Nombre, Apellido",
		Destino: "idColaborador, Nombre, Apellido",
		Clase: "F25_6",
	}
	ComboBusqueda(attrColaboradores);
	...
	tinymce.init({
		...
		setup: function(editor) {
			...
			inp.on("change",function(){
				...
				formData.append("Directorio", "Colaboraciones/"+idColaboracion);
				...
				$.ajax({
					...
					editor.insertContent('<img src="http://castioficial.com/CASTI/images/Colaboraciones/'+idColaboracion+"/"+id+'.'+resp.Tipo+'"/>');
					inp.val('');
				});
...
<form id="EditForm" name="EditForm" method="post" class="FormData" enctype="multipart/form-data" >
  <input type="hidden" name="idColaboracion" id="idColaboracion" value="<?php echo $idColaboracion; ?>">
  <div class="C6 C1080">
    <div class="FormDataField" id="cmbColaborador" valor="<?php echo (isset($_POST['id']) ? $row['idColaborador']:"") ?>"></div>
    ...
    <div class="FormTextareaField">
      <label class="DataName">Colaboracion:</label>
      <textarea rows="25" class="TextareaField F6 tinymce" name="NoticiaHTML" id="NoticiaHTML"><?php echo (isset($_POST['id']) ? utf8_encode($row['Noticia']) : ''); ?></textarea>
    </div>
    <div class="FormDataField">
...
<!--
    <div class="FormTextareaField ACenter">
      <?php
				...
				if (isset($row['TipoArchivo']) && is_file($srv."/images/Colaboraciones/".$idColaboracion.".".$row['TipoArchivo'])) { 
        	$src = "/".$_SESSION['Web']."/images/Colaboraciones/".$idColaboracion.".".$row['TipoArchivo']."?".time();
				} else {
