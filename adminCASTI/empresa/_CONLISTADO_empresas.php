...
<div id="ContenedorPrincipal">
    <h1><?php echo utf8_encode($rowPage['Observaciones']) ?></h1>
    <div id="Acciones">
      <ul class="Botones">
        ...
        <form id="fOrden" name="fOrden" action="<?php echo $_SERVER['PHP_SELF']."?".$queryString; ?>" method="post">
          <select name="qOrden">
            <option value="Razon Social"<?php if ("Cliente"==$_POST['qOrden']) {echo "SELECTED";} ?>>RAZON SOCIAL</option>
          </select>
      ...
      <div id="DatosBusqueda">
        <div class="BusquedaCol_1_3 SeparadorBusquedaCol">
          <div class="BusquedaParametro">
            ...
            <input name="bRazonSocial" type="text" id="bRazonSocial" value="<?php echo (isset($_GET['bRazonSocial']) ? $_GET['bRazonSocial'] :  NULL); ?>"/>
          </div>
				</div>
      </div>
    </form>
        ...
        <tbody class="scrollContent">
          <?php 
            ...
            while ($row = $rst->fetch_array()) {
              $i++; 
          ?>
              <tr class="RowData <?php echo ($i % 2 == 0 ? "RowOdd" : "RowPair") ?>" id="<?php echo htmlentities($row['CodEmpresa'], ENT_COMPAT, 'iso-8859-1'); ?>">
                <td style="width:200px"><?php echo htmlentities($row['CodEmpresa'],ENT_COMPAT,'iso-8859-1'); ?></td>
                <td style="width:1000px"><?php echo htmlentities($row['RazonSocial'],ENT_COMPAT,'iso-8859-1'); ?></td>
              </tr>
          <?php 
            }
            $rst->free();
